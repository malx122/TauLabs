 ...
 * @{
 * @brief Defines board specific static initializers for hardware for the Kfly board.
 *****************************************************************************/
/* 
...
void PIOS_Board_Init(void) {
	...
	if (boot_count < 3) {
		...
		HwKFlySetDefaults(HwKFlyHandle(), 0);
	...
	HwKFlyUSB_VCPPortGet(&hw_usb_vcpport);

	if (!usb_cdc_present) {
		/* Force VCP port function to disabled if we haven't advertised VCP in our USB descriptor */
		hw_usb_vcpport = HWKFLY_USB_VCPPORT_DISABLED;
	}

	switch (hw_usb_vcpport) {
	case HWKFLY_USB_VCPPORT_DISABLED:
		break;
	case HWKFLY_USB_VCPPORT_USBTELEMETRY:
		...
		break;
	case HWKFLY_USB_VCPPORT_COMBRIDGE:
#if defined(PIOS_INCLUDE_COM)
...
#endif	/* PIOS_INCLUDE_COM */
		break;
	case HWKFLY_USB_VCPPORT_DEBUGCONSOLE:
#if defined(PIOS_INCLUDE_COM)
#if defined(PIOS_INCLUDE_DEBUG_CONSOLE)
		{
	...
	HwKFlyUSB_HIDPortGet(&hw_usb_hidport);

	if (!usb_hid_present) {
		/* Force HID port function to disabled if we haven't advertised HID in our USB descriptor */
		hw_usb_hidport = HWKFLY_USB_HIDPORT_DISABLED;
	}

	switch (hw_usb_hidport) {
	case HWKFLY_USB_HIDPORT_DISABLED:
		break;
	case HWKFLY_USB_HIDPORT_USBTELEMETRY:
#if defined(PIOS_INCLUDE_COM)
		{
	...
	case HWKFLY_UART1_I2C:
		...
		break;
	case HWKFLY_UART1_DSM2:
	case HWKFLY_UART1_DSMX10BIT:
	case HWKFLY_UART1_DSMX11BIT:
		...
		break;
	case HWKFLY_UART1_DEBUGCONSOLE:
	...
	/* UART2 Port */
	uint8_t hw_uart2;
	HwKFlyUart2Get(&hw_uart2);
	switch (hw_uart2) {
	case HWKFLY_UART2_DISABLED:
		break;
	case HWKFLY_UART2_TELEMETRY:
#if defined(PIOS_INCLUDE_TELEMETRY_RF) && defined(PIOS_INCLUDE_USART) && defined(PIOS_INCLUDE_COM)
	...
	case HWKFLY_UART2_SBUS:
		...
		break;
	case HWKFLY_UART2_DSM2:
	case HWKFLY_UART2_DSMX10BIT:
	case HWKFLY_UART2_DSMX11BIT:
#if defined(PIOS_INCLUDE_DSM)
		{
			enum pios_dsm_proto proto;
			switch (hw_uart2) {
	...
	/* UART3 Port */
	uint8_t hw_uart3;
	HwKFlyUart3Get(&hw_uart3);
	switch (hw_uart3) {
	case HWKFLY_UART3_DISABLED:
		break;
	case HWKFLY_UART3_TELEMETRY:
#if defined(PIOS_INCLUDE_TELEMETRY_RF) && defined(PIOS_INCLUDE_USART) && defined(PIOS_INCLUDE_COM)
...
#endif	/* PIOS_INCLUDE_I2C */
		break;
	case HWKFLY_UART3_DSM2:
	case HWKFLY_UART3_DSMX10BIT:
	case HWKFLY_UART3_DSMX11BIT:
#if defined(PIOS_INCLUDE_DSM)
		{
			enum pios_dsm_proto proto;
			switch (hw_uart3) {
	...
	/* UART4 Port */
	uint8_t hw_uart4;
	HwKFlyUart4Get(&hw_uart4);
	switch (hw_uart4) {
	case HWKFLY_UART4_DISABLED:
		break;
	case HWKFLY_UART4_TELEMETRY:
#if defined(PIOS_INCLUDE_TELEMETRY_RF) && defined(PIOS_INCLUDE_USART) && defined(PIOS_INCLUDE_COM)
	...
	/* UART5 Port */
	uint8_t hw_uart5;
	HwKFlyUart5Get(&hw_uart5);
	switch (hw_uart5) {
	case HWKFLY_UART5_DISABLED:
		break;
	case HWKFLY_UART5_TELEMETRY:
#if defined(PIOS_INCLUDE_TELEMETRY_RF) && defined(PIOS_INCLUDE_USART) && defined(PIOS_INCLUDE_COM)
	...
	uint8_t hw_rcvrport;
	HwKFlyRcvrPortGet(&hw_rcvrport);

	switch (hw_rcvrport) {
	case HWKFLY_RCVRPORT_DISABLED:
		break;
	case HWKFLY_RCVRPORT_PWM:
		...
		break;
	case HWKFLY_RCVRPORT_PPM:
	case HWKFLY_RCVRPORT_PPMOUTPUTS:
